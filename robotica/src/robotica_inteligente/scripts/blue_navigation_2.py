Poco almacenamiento … Queda un 20% de tus 500 MB de espacio de almacenamiento individual. Para evitar interrupciones, libera espacio o contacta con tu administrador.
#!/usr/bin/env python
#
...
class BlueTrajectoryPlanner(object):
    ...
    def __init__(self):
        ...
        self.goal = geometry_msgs.msg.Point(x=3.9, y=4.2)  # Ejemplos de metas
        ...
        self.arrived_at_goal = rospy.Publisher(
            "/blue_arrived_at_goal",
            std_msgs.msg.Bool,
            queue_size=1
        )
    ...
    def calculate_angular_speed(self, speed, steer):
        omega = 0.0
        if steer != 0:
            R = VEHICLE_LENGTH / tan(steer)
    ...
    def controlActionCalculation(self):
        ...
        for steer in np.arange(-MAX_STEER_ANGLE, MAX_STEER_ANGLE + 0.01, self.delta_angle):
            ...
            for dir in directions:
                ...
                if not flag_collision_risk :#or (flag_collision_risk and collision_free_distance > 1.0):
                    #TODO calcular la puntuación de la trayectoria en base al error en distancia y ángulo al objetivo (self.local_path[-2]).
                    error = self.distance(self.local_path[-2], local_point) + self.angle(self.local_path[-2], local_point)
                    if error < min_error:
                        min_error = error
                        ackermann_control.steering_angle = steer
                        ackermann_control.speed = dir

        print(f"Best direction: Steering={ackermann_control.steering_angle}, Speed={ackermann_control.speed}")
        self.ackermann_command_publisher.publish(ackermann_control)
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            self.arrived_at_goal.publish(self.goal_reached)
            if count==0:

...
class BlueTrajectoryPlanner(object):
    ...
    def __init__(self):
        ...
        self.goal.x, self.goal.y = 5.9, 4.4
        self.local_path.append(self.goal)
        self.local_path.append(self.goal)
        # print("self.goal x: {}, y: {}".format(self.goal.x, self.goal.y))
        ...
        # NEW ----------------------------------------------
        self.objective_subscriber = rospy.Subscriber("/blue/loaded_object",
            std_msgs.msg.String, self.objective_callback, queue_size=1)
    ...
    def objective_callback(self, objective:std_msgs.msg.String):
        print("Objective received: ", objective.data)
    ...
    def position_callback(self, ground_truth:Odometry):
        ...
        # print("Ground_truth: ", ground_truth.pose.pose.orientation, type(ground_truth.pose.pose.orientation))
        # The type is a quaternion, so we need to convert it to euler angles
    ...
    def controlActionCalculation(self):
        ...
        if self.distance(self.position, self.goal) < self.reached_distance:
            ...
            self.goal_publisher.publish(std_msgs.msg.String("goal_reached"))
            return
        ...
        for steer in np.arange(-MAX_STEER_ANGLE, MAX_STEER_ANGLE+0.01, self.delta_angle):
            ...
            for dir in directions:
                ...
                #TODO Calcular la velocidad de giro
                if radius==0.0:
                    angular_speed = 0.0
                else:
                    angular_speed = speed2/radius
                ...
                for sample in np.arange(self.delta_sample, self.max_sample+0.01, self.delta_sample):
                    ...
                    # Make the TODO correctly
                    local_point.x =  dir*sample*cos(angular_speed*sample)
                    local_point.y =  dir*sample*sin(angular_speed*sample)
                ...
                if not flag_collision_risk:
                    ...
                    # Make the TODO
                    error = self.distance(self.local_path[-2], local_point) + self.theta* self.angle(self.local_path[-2], local_point)
                    if error < min_error:
                        min_error=error

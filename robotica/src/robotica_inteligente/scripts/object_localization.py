...
def img_xyz(centroid, depth, camera_matrix):
    ...
    # x = fx * (centroid[0]/depth) + cx
    # y = fy * (centroid[1]/depth) + cy
...
def color_image_callback(color_image_msg):
    ...
    filtered_obj_image_msg = bridge.cv2_to_imgmsg(filtered_obj, encoding="bgr8")
    filtered_obj_pub.publish(filtered_obj_image_msg)
...
def ros_node():
    ...
    camera_info_sub = rospy.Subscriber('/camera/color/camera_info', CameraInfo, camera_info_callback) # Callback correspondiente:  camera_info_callback
